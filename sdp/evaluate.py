...
def run_biological_validation(
    ...
    _bar_panel(axes[1], [ari2,  nmi2,  sil2],
               f"φ_j  k=2: Lymph vs Gran\n(N={N_2cls})")
    _bar_panel(axes[2], [ari3,  nmi3,  sil3],
               f"φ_j  k=3: All classes\n(N={N_labeled}, Mono n=2)")
    _bar_panel(axes[3], [ari2p, nmi2p, sil2p],
               f"ψ_j  k=2: Lymph vs Gran\n(N={N_2cls})")
    _bar_panel(axes[4], [ari3p, nmi3p, sil3p],
               f"ψ_j  k=3: All classes\n(N={N_labeled}, Mono n=2)")

    plt.suptitle(
        r"Biological validation: $\varphi_j \in \mathbb{R}^{CK}$ (concat codes)"
        r" vs $\psi_j \in \mathbb{R}^{K}$ (cross-channel atom norms)",
