...
# Single run-time timestamp used for all output filenames in this module.
...
def save_convergence_plots(
    ...
    fp = Path(output_dir) / f"convergence_{_TIMESTAMP}.png"
...
def save_single_unified_cell(
    ...
    fig_path = (pathlib.Path(out_dir) /
                f"unified_single_cell_{cell_idx}_{_TIMESTAMP}.png")
...
def save_unified_cell_figure(
    ...
    plt.tight_layout()
    fig_path = pathlib.Path(out_dir) / f"unified_cell_figure_{_TIMESTAMP}.png"
    plt.savefig(fig_path, dpi=300, bbox_inches="tight", facecolor="white")
    plt.close()
...
def save_reconstructed_images(
    ...
    outpath = pathlib.Path(out_dir) / f"reconstructed_image_{_TIMESTAMP}.png"
...
def save_unified_vs_truth(
    ...
    for k, name in label_names.items():
        ...
        fig = plt.figure(figsize=(n_avail * cell_w, 2 * cell_h + 1.2))
        fig.suptitle(
            f"{name}  -  {n_avail} cells  |  "
            f"Row 0: {gt_channel} GT    Row 1: u_j (weighted average of per-channel reconstructions)",
            fontsize=14, fontweight="bold", x=0.01, ha="left", y=0.99,
        )
        gs = GridSpec(2, n_avail, figure=fig, hspace=0.10, wspace=0.12,
                      top=0.85, bottom=0.06, left=0.10, right=0.99)
        ...
        for col, ds_idx in enumerate(cell_ds_idxs):
            ...
            ax0.set_title(f"idx {ds_idx}", fontsize=12, pad=4)
            if col == 0:
                ax0.set_ylabel(f"GT ({gt_channel})", fontsize=13,
                               fontweight="bold", rotation=90, labelpad=6, va="center")
            ...
            if col == 0:
                ax1.set_ylabel("u_j = Σ w_c D^(c)a^(c)", fontsize=13,
                               fontweight="bold", rotation=90, labelpad=6, va="center")
    ...
    out = pathlib.Path(out_dir) / f"unified_vs_truth_{gt_channel}_{_TIMESTAMP}.png"
